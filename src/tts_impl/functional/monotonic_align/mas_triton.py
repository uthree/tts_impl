...
@torch.no_grad()
def maximum_path_triton(value, mask, dtype=torch.float32):
    ...
    path = _maximum_path_triton(path, value, t_x_max, t_y_max)
    return path


@triton.jit
def _maximum_path(
    path, value, t_x, t_y, B, T, S, max_neg_val, BLOCK_SIZE_X: tl.constexpr
):
    ...
    for j in range(1, y_length, 1):
        v_cur = tl.load(
            value + (offs_prev) * S + (j - 1),
            mask=(offs_prev < x_length),
            other=max_neg_val,
        )
        v_prev = tl.load(
            value + (offs_prev - 1) * S + (j - 1),
            mask=(0 < offs_prev) & (offs_prev < x_length),
            other=max_neg_val,
        )
        # compare v_cur and v_prev, and update v with larger value
        v = tl.maximum(v_cur, v_prev) + tl.load(
            value + (offs_prev) * S + j, mask=(offs_prev < x_length)
        )
...
@torch.no_grad()
def _maximum_path_triton(path, value, t_x, t_y, max_neg_val=-1e32):
    B, T, S = path.shape
