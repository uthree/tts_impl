...
class NhvcLayer(StatefulModule):
    ...
    def _parallel_forward(self, x: torch.Tensor, h_prev: torch.Tensor, g: Tensor | None=None, *args, **kwargs) -> Tuple[Tensor, Tensor]:
...
class NhvcEncoder(StatefulModule):
    ...
    """
    def __init__(self, d_model: int=128, n_layers: int=4, n_fft: int = 1024, dim_phonemes:int= 64, n_f0_classes: int = 128):
        super().__init__()
        self.fft_bin = n_fft // 2 + 1
        self.pre = nn.Linear(self.fft_bin, d_model)
        self.stack = StatefulModuleSequential([NhvcLayer(d_model) for _ in range(n_layers)])
        self.post = nn.Linear(d_model, dim_phonemes + n_f0_classes + self.fft_bin)

    def _initial_state(self, x: Tensor, *args, **kwargs) -> Tensor:
        return self.stack._initial_state(x, *args, **kwargs)
    
    def _parallel_forward(self, x: Tensor, h: Tensor, *args, **kwargs) -> Tuple[Tensor, Tensor]:
        x = self.pre(x)
...
class NhvcDecoder(StatefulModule):
    ...
    """
    def __init__(self, d_model: int=128, n_layers: int=4, dim_phonemes:int=64, n_fft: int = 1024, dim_periodicity: int=16, gin_channels: int=0):
        super().__init__()
        self.fft_bin = n_fft // 2 + 1
        self.pre = nn.Linear(dim_phonemes, d_model)
        self.stack = StatefulModuleSequential([NhvcLayer(d_model, gin_channels=gin_channels) for _ in range(n_layers)])
        self.post = nn.Linear(d_model, dim_periodicity + self.fft_bin)

    def _initial_state(self, x: Tensor, *args, **kwargs) -> Tensor:
        return self.stack._initial_state(x, *args, **kwargs)
    
    def _parallel_forward(self, x: Tensor, h: Tensor, g: Tensor|None=None *args, **kwargs) -> Tuple[Tensor, Tensor]:
        x = self.pre(x)
        x, h_last = self.stack._parallel_forward(x, h, g=g, *args, **kwargs)
        x = self.post(x)
        return x, h_last
