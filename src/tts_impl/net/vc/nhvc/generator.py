...
class NhvcGenerator(nn.Module):
    ...
    def forward(self, spec, f0, ssl, sid):
        ...
        z, f0_logits, noise_filter = torch.split(
            enc_output,
            [
                self.encoder.dim_phonemes,
                self.encoder.n_f0_classes,
                self.encoder.fft_bin,
            ],
            dim=2,
        )
        ...
        dec_output, _ = self.decoder.forward(z.detach(), g=g.unsqueeze(1))
        per, env = torch.split(
            dec_output, [self.decoder.dim_periodicity, self.decoder.fft_bin], dim=2
        )
