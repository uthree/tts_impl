...
class NhvcLightningModule(LightningModule):
    ...
    def _test_or_validate_batch(self, batch, bid):
        ...
        for i in range(fake.shape[0]):
            ...
            c = cvt[i].sum(dim=0, keepdim=True).detach().cpu()
            ...
            self.logger.experiment.add_audio(
                ...
                f,
                self.current_epoch,
                sample_rate=self.generator.vocoder.sample_rate,
            )
            self.logger.experiment.add_audio(
            ...
            self.logger.experiment.add_audio(
                f"converted waveform/{bid}_{i}_{sid_n}_to_{tid_n}",
                c,
                self.current_epoch,
                sample_rate=self.generator.vocoder.sample_rate,
            )
