...
class HarmonicNoiseOscillator(Oscillator):
    ...
    def forward(self, f0, uv):
        ...
        with torch.no_grad():
            ...
            source = (source * F.normalize(self.weights, p=2.0, dim=1)).sum(
                dim=1, keepdim=True
            )
        return source


class ImpulseOscillator(Oscillator):
    def __init__(
        self,
        sample_rate,
        frame_size,
...
class CyclicNoiseOscillator(Oscillator):
