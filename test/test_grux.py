...
def test_dwc_conv(d_model, kernel_size, batch_size, seq_len, num_layers):
    model = Grux(d_model, num_layers=num_layers, kernel_size=kernel_size)
...
@pytest.mark.parametrize("seq_len", [1, 10, 20])
@pytest.mark.parametrize("num_layers", [1, 4])
def test_mingru_sanity_check(d_model, kernel_size, batch_size, seq_len, num_layers):
    model = Grux(d_model, num_layers=num_layers, kernel_size=kernel_size)
    x = torch.randn(batch_size, seq_len, d_model)
    y_par, _ = model(x)
